-- BQ5: shared personalized Sidequest recommendation.
-- Used by both Kotlin and Flutter so ranking logic is not duplicated per client.
...
revoke all on function public.recommend_quests(integer, text, text[], text, integer)
from public, anon;

grant execute on function public.recommend_quests(integer, text, text[], text, integer)
to authenticated;
